#!/usr/bin/env python3
"""══════════════════════════════════════════════════════════════════════════════
MolProp Toolkit Calculator (v4)
═══════════════════════════════════════════════════════════════════════════════

This calculator creates a results CSV intended to be consumed by the analyzers
in this repository. The main design goal is that the first columns are the ones
most medicinal chemists want to see first: CNS MPO, oral bio filters, Rule of 5
and Rule of 3, medchem flags, toxicity alerts, and metabolic soft spots.

The calculator is RDKit-based. It does not require Mordred. The output is still
compatible with the analyzers even if you later extend it with additional
columns.

Usage examples:

  python calculators/mpo_v4.py input.smi -o results.csv
  python calculators/mpo_v4.py input.smi -o results.csv --fill-missing 0

Disclaimer: several sections are rule/alert based heuristics for triage.
"""
...
# ---------- SMARTS dictionaries (alerts and soft spots) ----------
...
def calc_cyp(mol: Chem.Mol, logp: float) -> Dict[str, bool | int]:
    ...
    pyr = has_smarts(mol, CYP_FEATURES["pyridine"])
...
def compute(
    ...
    """Compute properties after optional structure preparation.

    Ionization handling currently supports a lightweight heuristic mode that
    outputs pH-dependent features (Ion_* columns). It does not modify the
    structure used for descriptor calculation.
    """
    ...
    # Prepare (standardize parent, optional prep-tautomer canonicalization, stereo reporting)
    mol = mol_in
    prep_report = None
    ...
    # Optional tautomer enumeration step (Task 3)
    calc_mol = mol
    taut_rep = None
    ...
    # Base (pre-protomer) SMILES used for optional protomer enumeration
    ...
    # Task 2B: optional protomer enumeration/selection (Dimorphite-DL)
    prot_rep = None
    if protomer_cfg is not None and protomer_cfg.mode.lower() == "dimorphite":
        ...
        # Only compute descriptors on protomer if explicitly requested.
        if protomer_cfg.calc_on_protomer and prot_mol is not None:
            calc_mol = prot_mol

    # Descriptor calculation structure
    ...
    # Traceability and preparation metadata
    out["Input_Canonical_SMILES"] = input_can

    # Canonical_SMILES is the prepared molecule SMILES.
    out["Canonical_SMILES"] = prep_can

    # Calc_Canonical_SMILES is the structure actually used for descriptor calculation.
    out["Calc_Canonical_SMILES"] = Chem.MolToSmiles(calc_mol, canonical=True, isomericSmiles=True)

    # Base SMILES before protomerization (tautomer+stereo-selected structure)
    ...
    # Ionization / pH features (heuristic model output). We emit these for both
    # 'heuristic' and 'dimorphite' modes because they are useful for triage and
    # for protomer selection logic.
...
def write_csv(rows: List[Dict[str, object]], out_path: str, fill_missing: Optional[float]) -> None:
    ...
    # Preserve a stable first-column ordering, then append remaining keys.
    preferred = [
        "Compound_ID",
        "SMILES",
    ...
    if fill_missing is not None:
        for r in rows:
            for c in header:
                if c not in r or r[c] is None or (isinstance(r[c], float) and (math.isnan(r[c]) or math.isinf(r[c]))):
                    r[c] = fill_missing
...
def main() -> None:
    ...
    # Backwards compatibility: --no-tautomer implies tautomer-mode=none unless explicitly overridden.
    ...
    # Protomer (Dimorphite-DL) configuration
