...
If you need a machine-readable manifest of column types, units, and short guidance, see `docs/schema.json`. You can validate a results table with `molscope schema` and you can generate a schema-derived column reference doc (`docs/columns_schema.md`) with `tools/generate_columns_md_from_schema.py`.
