...
`molscope similarity` provides fingerprint-based similarity searching for molecular libraries. It supports multiple fingerprint types, similarity metrics, and operational modes including batch search, pairwise matrices, diversity picking, and clustering.

## Quick Start

```bash
# Basic similarity search
molscope similarity "CCO" library.smi -o similar_to_ethanol.csv

# Find top 10 compounds similar to benzene with Tanimoto ≥ 0.7
molscope similarity "c1ccccc1" library.csv --fp morgan --threshold 0.7 --top 10

# List available fingerprint types
molscope similarity --list-fps

# List available similarity metrics
molscope similarity --list-metrics
```
...
```bash
# Basic search
molscope similarity "CC(=O)Oc1ccccc1C(=O)O" results.csv --top 20

# With threshold filter
molscope similarity "CC(=O)Oc1ccccc1C(=O)O" results.csv --threshold 0.6

# Using different fingerprint
molscope similarity "CC(=O)Oc1ccccc1C(=O)O" results.csv --fp maccs

# Using different metric
molscope similarity "CC(=O)Oc1ccccc1C(=O)O" results.csv --metric dice

# Generate HTML report
molscope similarity "CC(=O)Oc1ccccc1C(=O)O" results.csv --top 50 -o results.csv --html
```
...
molscope similarity queries.smi library.csv --batch -o results/

# With all options
molscope similarity queries.smi library.csv --batch --fp morgan --threshold 0.5 --top 10 -o results/ --html
```
...
```bash
molscope similarity --pairwise library.smi -o similarity_matrix.csv

# With specific fingerprint
molscope similarity --pairwise library.smi --fp maccs -o matrix_maccs.csv
```
...
molscope similarity --diversity library.smi --pick 100 -o diverse_100.csv

# With specific seed compound
molscope similarity --diversity library.smi --pick 50 --seed "c1ccccc1" -o diverse_50.csv
```
...
molscope similarity --cluster library.smi --threshold 0.7 -o clusters.csv

# More stringent clustering
molscope similarity --cluster library.smi --threshold 0.85 -o clusters_strict.csv
```
...
```bash
molscope similarity "CCO" library.smi --knn 5
```
...
molscope similarity "CCO" library.smi --fp morgan --radius 3 --nbits 1024

# FCFP4 equivalent
molscope similarity "CCO" library.smi --fp morgan_feat --radius 2
```
...
### MolScope Results Tables

Works directly with output from `molscope calc`:

```bash
molscope calc library.smi -o results.csv
molscope similarity "CCO" results.csv --smiles-col Canonical_SMILES
```
...
molscope similarity "CCO" library.smi -j 4

# Use all CPU cores
molscope similarity "CCO" library.smi -j -1

# Show progress bar (requires tqdm)
molscope similarity "CCO" library.smi --progress
```
...
molscope similarity "$LEAD_SMILES" library.csv --threshold 0.5 --top 100 -o lead_analogs.csv
...
molscope similarity --cluster library.smi --threshold 0.95 -o clusters.csv
...
molscope similarity --pairwise library.smi -o matrix.csv

# Diversity picking for coverage analysis
molscope similarity --diversity library.smi --pick 100 -o diverse_subset.csv
```
...
molscope similarity "$COMPOUND" library.csv --threshold 0.8 -o close_analogs.csv --html
```
