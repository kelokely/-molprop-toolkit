...
This document explains what each analysis category in MolScope Toolkit is trying to capture, why the category exists in early drug discovery, how to interpret the common thresholds, and how to use the output to decide what to do next. The analyzers are intentionally pragmatic. They are meant to help you find compounds that are clean and promising, compounds that are likely to create trouble later, and compounds that are worth optimizing because the issues are addressable.

## Preclinical profiling

One useful way to read the toolkit is as a lightweight preclinical profiling stack built from structure-derived heuristics. The categories below naturally split into in vitro-relevant modules and in vivo-relevant modules, which makes it easier to decide what should be used for assay planning versus what should be used for exposure and developability planning.

The in vitro-relevant modules are the categories that most directly support early screening decisions: med-chem flags, toxicity alerts, metabolism and soft spots, hERG liability, CYP inhibition burden, solubility, and permeability. These categories help answer which compounds should move into counterscreens, stability assays, permeability assays, or more focused liability work first.

The in vivo-relevant modules are the categories that summarize likely PK and developability headwinds: pharmacokinetics heuristics, oral bioavailability filters, protein-binding and distribution tendencies, clearance risk, and composite developability indices. These categories are best used to compare compounds or scaffolds before committing to more expensive PK, formulation, or efficacy studies.
