...
from molprop_toolkit.decision_presets import STAGE_CHOICES, get_stage_preset
from molprop_toolkit.fingerprints import (
    FingerprintSpec,
...
from molprop_toolkit.picklists.html import (
    StructureAssetSpec,
    copy_structure_assets,
    write_index_html,
    write_picklist_html,
)
...
def _resolve_definitions(
    *,
    ...
    if not picklist_keys:
        if stage:
            preset = get_stage_preset(stage)
            selected: List[PicklistDefinition] = []
            seen: set[str] = set()
            if preset is not None:
                for key in preset.recommended_picklists:
                    if key in all_defs and key not in seen:
                        selected.append(all_defs[key])
                        seen.add(key)
            for definition in config_defs:
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=None,
        help="Optional campaign stage preset that selects a stage-appropriate default subset of built-in picklists.",
    )
    ap.add_argument(
        "--outdir",
    ...
    defs = _resolve_definitions(
        picklist_keys=picklist_keys,
    ...
    # Run picklists
    _, _, results = build_picklists_from_df(df_full, defs, stage=args.stage)
    ...
    for res in results:
        ...
        if args.columns == "all":
            ...
            for c in DECISION_OUTPUT_COLUMNS:
                if c in res.df.columns and c not in cols:
                    cols.append(c)
            df_out = res.df[[c for c in cols if c in res.df.columns]].copy()
        else:
            cols = select_output_columns(res.df, id_col, d)
            df_out = res.df[[c for c in cols if c in res.df.columns]].copy()
        ...
        if args.html:
            ...
            if args.stage:
                preset = get_stage_preset(args.stage)
                if preset is not None:
                    subtitle += f" • stage: {preset.label}"
            summary_rows = [
                ("queue", d.queue or d.key),
                ("next_action", d.next_action or "Review this queue with project context."),
    ...
    if args.html:
        subtitle = (
            f"Source: {Path(csv_path).name} • Generated: {datetime.now().isoformat(timespec='seconds')}"
        )
        if args.stage:
            preset = get_stage_preset(args.stage)
            if preset is not None:
                subtitle += f" • Stage preset: {preset.label}"
        write_index_html(
    ...
    for k, _name, count, href in index_entries:
