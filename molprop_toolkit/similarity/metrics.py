...
def tanimoto_similarity(fp1, fp2) -> float:
    ...
    if (
        HAS_RDKIT
        and not isinstance(fp1, np.ndarray)
        and not isinstance(fp2, np.ndarray)
    ):
...
def bulk_similarity(query_fp, target_fps, metric: str = "tanimoto") -> np.ndarray:
    ...
    if (
        HAS_RDKIT
        and _is_rdkit_bitvect(query_fp)
        and all((fp is None) or _is_rdkit_bitvect(fp) for fp in target_fps)
    ):
