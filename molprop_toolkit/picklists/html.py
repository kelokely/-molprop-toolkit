...
def _html_table(headers: Sequence[str], rows: Sequence[Tuple[str, str]]) -> str:
    ths = "".join(f"<th>{_html.escape(str(h))}</th>" for h in headers)
    trs = []
    for row in rows:
    ...
    *,
    ...
    if summary_rows:
        summary_block = (
            ...
            + _html_table(["item", "value"], summary_rows)
        )
